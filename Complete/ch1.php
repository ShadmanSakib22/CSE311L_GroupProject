...
if (isset($_POST['submit'])) {
    ...
    $sql = "INSERT INTO comments1(name, msg, dtime) VALUES ('$username', '$msg', '$date')";
...
<body>
    ...
    <div class="ses">
        <?php
        $sql = "SELECT name, dtime, msg, Cid FROM comments1 ORDER BY Cid DESC LIMIT 0,3";
        $result = mysqli_query($conn, $sql);
        ...
        if (isset($_POST['showMore'])) {
            $sql = "SELECT name, dtime, msg, Cid FROM comments1 ORDER BY Cid DESC LIMIT 3,50";
            $result = mysqli_query($conn, $sql);
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<div class='commentBox'>";
                echo "<ses style='color:yellowgreen'>" . $row["name"] . '<br></ses>';
